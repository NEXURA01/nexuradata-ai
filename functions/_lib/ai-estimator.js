...
const normalizeEstimate = (value) => {
  ...
  const band = shouldPromoteInfrastructure(scores) && scores.total < 26
    ? scoreBands[4]
    : scoreBandFromTotal(scores.total);
  const recommendedScope = normalizeText(estimate.recommended_scope, 120) || band.scope;
  const estimatedMinCad = normalizeCadAmount(estimate.estimated_min_cad, band.minCad);
  const estimatedMaxCad = Math.max(
    estimatedMinCad,
    normalizeCadAmount(estimate.estimated_max_cad, band.maxCad)
  );
