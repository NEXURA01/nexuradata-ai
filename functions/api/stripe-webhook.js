...
import { sendClientOperationalPaymentFollowUpEmail, sendTeamPaymentCompletedEmail } from "../_lib/email.js";
  ...
  try {
    ...
    try {
      ...
      if (operationalPayment?.id && isPaidEvent(event.type)) {
        ...
        const clientDelivery = await sendClientOperationalPaymentFollowUpEmail(
          context.env,
          {
            payment: operationalPayment,
            workflowCase,
        ...
        if (!clientDelivery.sent && clientDelivery.reason !== "not-configured" && clientDelivery.reason !== "missing-client-email") {
          console.error(JSON.stringify({
            timestamp: new Date().toISOString(),
            context: "client_payment_follow_up_not_sent",
            eventId: event.id,
            eventType: event.type,
            reason: clientDelivery.reason
          }));
        }
      }
    } catch {
